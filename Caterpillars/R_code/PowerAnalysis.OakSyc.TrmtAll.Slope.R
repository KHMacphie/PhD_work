...
#2: all samples on each tree, keep treesp in model ********** This script
...
slope <- difference[x]/2 #slope is per day so divide difference per 2 days by 2
oak.intercept <- intercept+0.45 #oak random effect in TTHA
syc.intercept <- intercept+0.12 #syc random effect in TTHA
d2.oak.mean <- d4.oak.mean+log(-2*slope)   
d2.syc.mean <- d4.syc.mean-2*log(slope) 
d6.oak.mean <- d4.oak.mean+2*log(slope) 
d6.syc.mean <- d4.syc.mean+2*log(slope) 
d8.oak.mean <- d4.oak.mean+4*log(slope) 
d8.syc.mean <- d4.syc.mean+4*log(slope) 
d10.oak.mean <- d4.oak.mean+6*log(slope) 
d10.syc.mean <- d4.syc.mean+6*log(slope) 

#simulating the response, running model and storing relevant outputs  
df$poisresponse <- c(
    rpois(n.beat.oak,  exp(d2.oak.mean + df.d2.oak$eff.date + df.d2.oak$eff.branch + df.d2.oak$eff.tree + df.d2.oak$eff.treedate + df.d2.oak$eff.branchdate)), 
    rpois(n.beat.syc,  exp(d2.syc.mean + df.d2.syc$eff.date + df.d2.syc$eff.branch + df.d2.syc$eff.tree + df.d2.syc$eff.treedate + df.d2.syc$eff.branchdate)))
  ...
  poismodel <- MCMCglmm(poisresponse~treatment+treesp, random=~date+branch+tree+treedate, family = "poisson", data = df, nitt=500000, burnin=50000, thin=20, prior=prior)
...
names <- c(paste(treecount,"Oak",treecount,"Syc_",difference,"dif"))
mypath <- file.path(paste("~/Documents/Models/PowerAnalysis/OakSyc_TrmtAll_",names[x],".pdf"))
...
mtext(paste("Oak trees=",n.oak,"   Sycamore trees=",n.syc,"   Prop difference (2days)=",propdif,"   Nsim=",repeats),side = 3, line = -1, outer = TRUE)
