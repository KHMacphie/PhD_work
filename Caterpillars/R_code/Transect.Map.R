...
sites.map <- ggplot(sites, aes(x = Mean.Long, y = Mean.Lat, color=Mean.Elev)) +
    ...
    geom_point(alpha = 0.9, size = 2.3)+#, colour = "black"))  # alpha controls the transparency, 1 is fully transparent
...
Habitat_FS_long <- gather(Habitat_FS, key="Taxon", value="FS", select=2:13)
