...
#### Equations.. ####
#Mass: intM + TTintM + dateM + TTdateM + date^2M
#Abund: intA + TTintA + dateA + TTdateA + date^2A + TTdate^2A

#Becasue Poisson just add the two equations then exp
#Biomass: intM + TTintM + dateM + TTdateM + date^2M + intA + TTintA + dateA + TTdateA + date^2A + TTdate^2A
#Biomass: intM+TTintM+intA+TTintA + dateM+TTdateM+dateA+TTdateA + date^2M+date^2A+TTdate^2A
...
#### pretending from one bivariate model (combining PD for each model) ####
...
Curveslong <- gather(Curves, key="TreeTaxa", value="Biomass", select= 2:12)
...
#write.csv(BMcurves,'~/Documents/Models/Tables/BMCurvesMetrics.csv')


# Peak Date
#PDplot <- ggplot(BMcurves, aes(fct_rev(TT), PD, colour=TT))+
...
# Peak Height
#PHplot <- ggplot(BMcurves, aes(fct_rev(TT), PH, colour=TT))+
#  geom_point(size=3, alpha=0.5)+
...
# Peak Width 
#PW.5plot <- ggplot(BMcurves, aes(fct_rev(TT), PW.5))+#, colour=TT))+
...
# Peak Width 
#PWplot <- ggplot(BMcurves, aes(fct_rev(TT), PW, colour=TT))+
#  geom_point(size=3, alpha=0.5)+
...
# Peak Width difference from mean
PWdifplot <- ggplot(BMcurves, aes(fct_rev(TT), PWdif))+#, colour=TT))+
...
#write.csv(BMODcurves,'~/Documents/Models/Tables/BMODCurvesMetrics.csv')
NoOakCols <- c("darkred", "firebrick3", "chocolate2", "goldenrod", "olivedrab4", "darkgreen", "royalblue4", "slateblue2", "orchid")

PDODplot <- ggplot(BMODcurves, aes(fct_rev(TT), PD))+#, colour=TT))+
  geom_point(size=2, alpha=0.9)+
  geom_errorbar(aes(ymax=PDUCI, ymin=PDLCI, width=0.5))+
  theme_bw()+
  xlab("Tree Taxon")+
...
PHpropODplot <- ggplot(BMODcurves, aes(fct_rev(TT), PHprop))+#, colour=TT))+
  geom_point(size=2, alpha=0.9)+
  geom_errorbar(aes(ymax=PHpropUCI, ymin=PHpropLCI, width=0.5))+
  theme_bw()+
  xlab("")+
...
PWODplot <- ggplot(BMODcurves, aes(fct_rev(TT), PW))+#, colour=TT))+
  geom_point(size=2, alpha=0.9)+
  geom_errorbar(aes(ymax=PWUCI, ymin=PWLCI, width=0.5))+
  theme_bw()+
  xlab("")+
...
PWOD.5plot <- ggplot(BMODcurves, aes(fct_rev(TT), PW.5))+#, colour=TT))+
  geom_point(size=2, alpha=0.9)+
  geom_errorbar(aes(ymax=PW.5UCI, ymin=PW.5LCI, width=0.5))+
  theme_bw()+
  xlab("")+
...
BMPW <- MeanPW.5 #using biomass pw.5
...
#### plot two mean curves ####
dayscal <- seq(-2.071,2.014,0.001)
days <- unscale(dayscal, center= 146.4095, scale=14.19835)
days <- days$V1

MeanCurve <- data.frame(dayscal= seq(-2.071,2.014,0.001))
MeanCurve$days <- unscale(MeanCurve$dayscal, center= 146.4095, scale=14.19835)
MeanCurve$days <- MeanCurve$days$V1

MeanCurve$Ab <- mean(sampleATTC[,2]) + mean(sampleATTC[,3])*MeanCurve$dayscal + mean(sampleATTC[,4])*MeanCurve$dayscal^2
MeanCurve$Bm <- mean(sampleATTC[,2] + sampleMass[,2]) + mean(sampleATTC[,3] + sampleMass[,5])*MeanCurve$dayscal + mean(sampleATTC[,4] + sampleMass[,6])*MeanCurve$dayscal^2

par(mar=c(4,4,3,3),oma=c(0,1,0,3)) # 6"x6"
plot(MeanCurve$days, exp(MeanCurve$Ab), type="l", lwd=2, ylim=c(0,0.06), xlab="Ordinal Date", ylab="Abundance")
par(new = T)
plot(MeanCurve$days, exp(MeanCurve$Bm), type="l", lwd=2, col="grey",axes=F,xlab=NA, ylab=NA, ylim=c(0,0.001763578))
axis(side = 4)
mtext(side = 4, line = 3, expression("Biomass (g)"))
legend("topleft", legend=c("Abundance","Biomass"), lwd=c(2,2), pch=c(NA, NA), col=c("black", "grey"),seg.len=1.2, bty = "n")
