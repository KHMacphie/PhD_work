...
maxtrees <- 60
intervals <- 10
treecount <- rep(seq(mintrees/5,maxtrees/5,intervals/5),2)
difference <- c(rep(0.333,(length(treecount)/2)),rep(0.5,length(treecount)/2))
...
reps.oak <- treecount[x] # (so reps=2 = 10 trees)                                        **********
#reps.syc <- 3 #                                                                         **********
propdif <- difference[x] #proportional difference per 2 day difference in treatment      **********
...
df$poisresponse <- c(
  ...
  poismodel <- MCMCglmm(poisresponse~treatment, random=~date+branch+tree+treedate, family = "poisson", data = df, nitt=600000, burnin=60000, prior=prior)
...
names <- c(paste(treecount*5,"trees_",difference,"propdif"))
mypath <- file.path(paste("~/Documents/Models/PowerAnalysis/Oak_TrmtSpread_",names[x],".pdf"))
