...
#YearCurves<- MCMCglmm(caterpillars~ datescaled*year + I(datescaled^2)*year, 
#                                 random=~ us(1+datescaled+I(datescaled^2)):site + us(1+datescaled):siteyear + treeID + siteday + recorder, 
#                                 family="poisson", data=cater_habitat, prior=prior3, nitt=300000, burnin=30000)
#save(YearCurves, file = "~/Documents/Models/YearCurves.RData")
load("~/Documents/Models/YearCurves.RData")

pd19 <- -YearCurves$Sol[,2]/(2*YearCurves$Sol[,8])
pd19 <- unscale(pd19, center= 146.4095, scale=14.19835)
pd19 <- pd19$var1

pd18<- -(YearCurves$Sol[,2]+YearCurves$Sol[,13])/(2*(YearCurves$Sol[,8]+YearCurves$Sol[,18]))
pd18 <- unscale(pd18, center= 146.4095, scale=14.19835)
pd18 <- pd18$var1

pd17<- -(YearCurves$Sol[,2]+YearCurves$Sol[,12])/(2*(YearCurves$Sol[,8]+YearCurves$Sol[,17]))
pd17 <- unscale(pd17, center= 146.4095, scale=14.19835)
pd17 <- pd17$var1

pd16<- -(YearCurves$Sol[,2]+YearCurves$Sol[,11])/(2*(YearCurves$Sol[,8]+YearCurves$Sol[,16]))
pd16 <- unscale(pd16, center= 146.4095, scale=14.19835)
pd16 <- pd16$var1

pd15<- -(YearCurves$Sol[,2]+YearCurves$Sol[,10])/(2*(YearCurves$Sol[,8]+YearCurves$Sol[,15]))
pd15 <- unscale(pd15, center= 146.4095, scale=14.19835)
pd15 <- pd15$var1

pd14<- -(YearCurves$Sol[,2]+YearCurves$Sol[,9])/(2*(YearCurves$Sol[,8]+YearCurves$Sol[,14]))
pd14 <- unscale(pd14, center= 146.4095, scale=14.19835)
pd14 <- pd14$var1

mean(pd14) #155.8757
HPDinterval(mcmc(pd14)) #150.118 162.9168
mean(pd15) #161.456
HPDinterval(mcmc(pd15)) #157.5546 165.8864
mean(pd16) #159.4314
HPDinterval(mcmc(pd16)) #157.1615 161.8944
mean(pd17) #150.9728
HPDinterval(mcmc(pd17)) #147.7368 154.2906
mean(pd18) #153.4515
HPDinterval(mcmc(pd18)) #151.8148 155.1767
mean(pd19) #147.041
HPDinterval(mcmc(pd19)) #144.7748 149.3704
