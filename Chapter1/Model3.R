...
plot(CurveShape_scaled) #look at fixed effect and random term trace plots 
CurveShape_scaled.Sim<-simulate(CurveShape_scaled,nsim=1000) #simulate 1000 times
par(mfcol=c(1,1))
hist(apply(CurveShape_scaled.Sim,2,sum), breaks=1000) #histogram of simulation predictions for total abundance
...
hist(apply(CurveShape_scaled.Sim,2,propzero), breaks=100) # histogram of proportion of zeros in simulated data
...
cdf$root1.5 <- ggfortify::unscale((apply(roots.5, 1, min, na.rm=TRUE)), center= 146.4095, scale=14.19835)  
cdf$root2.5 <- ggfortify::unscale((apply(roots.5, 1, max, na.rm=TRUE)), center= 146.4095, scale=14.19835) 

#days either side of peak
cdf$left.5 <- ggfortify::unscale(cdf$pd, center= 146.4095, scale=14.19835)-cdf$root1.5
cdf$left.5 <- cdf$left.5$var1
cdf$right.5 <- cdf$root2.5-(ggfortify::unscale(cdf$pd, center= 146.4095, scale=14.19835))
...
cdf$root1.25 <- ggfortify::unscale((apply(roots.25, 1, min, na.rm=TRUE)), center= 146.4095, scale=14.19835)   
cdf$root2.25 <- ggfortify::unscale((apply(roots.25, 1, max, na.rm=TRUE)), center= 146.4095, scale=14.19835) 

cdf$left.25 <- ggfortify::unscale(cdf$pd, center= 146.4095, scale=14.19835)-cdf$root1.25
cdf$left.25 <- cdf$left.25$var1
cdf$right.25 <- cdf$root2.25-ggfortify::unscale(cdf$pd, center= 146.4095, scale=14.19835)
...
cdf$root1.75 <- ggfortify::unscale((apply(roots.75, 1, min, na.rm=TRUE)), center= 146.4095, scale=14.19835)   
cdf$root2.75 <- ggfortify::unscale((apply(roots.75, 1, max, na.rm=TRUE)), center= 146.4095, scale=14.19835)  

cdf$left.75 <- ggfortify::unscale(cdf$pd, center= 146.4095, scale=14.19835)-cdf$root1.75
cdf$left.75 <- cdf$left.75$var1
cdf$right.75 <- cdf$root2.75-ggfortify::unscale(cdf$pd, center= 146.4095, scale=14.19835)
...
for(i in 1:2599){ # sooo many iterations plotting just 1 in every 15
