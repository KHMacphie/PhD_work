...
#ATTC20<- MCMCglmm(caterpillars~ datescaled + I(datescaled^2), 
...
#                  family="poisson", data=cater_habitat, prior=prior3, nitt=1050000, burnin=50000, pr=TRUE, thin=100)
#save(ATTC20, file = "~/Dropbox/Kirsty's/Chapter1/Models/Inc2020/ATTC20.RData") #sample size 10000
load("~/Dropbox/Kirsty's/Chapter1/Models/Inc2020/ATTC20.RData")
summary(ATTC20)

#### Checking model fits the data and converged ####
plot(ATTC20) #look at fixed effect and random term trace plots 
ATTC20.Sim<-simulate(ATTC20,nsim=1000) #simulate 1000 times
par(mfcol=c(1,1))
hist(apply(ATTC20.Sim,2,sum), breaks=1000) #histogram of simulation predictions for total abundance
#normally not too many rogue values but reduce x axis a bit to see main distribution relative to observed value 
hist(apply(ATTC20.Sim,2,sum), breaks=10000, xlim=c(0,100000))
...
hist(apply(ATTC20.Sim,2,propzero), breaks=100) # histogram of proportion of zeros in simulated data
...
AlderC <- (ATTC20$Sol[,1] + ATTC20$Sol[,4])
AshC <- (ATTC20$Sol[,1] + ATTC20$Sol[,5])
BeechC <- (ATTC20$Sol[,1] + ATTC20$Sol[,6])
BirchC <- (ATTC20$Sol[,1] + ATTC20$Sol[,7])
ElmC <- (ATTC20$Sol[,1] + ATTC20$Sol[,8])
HazelC <- (ATTC20$Sol[,1] + ATTC20$Sol[,9])
OakC <- (ATTC20$Sol[,1] + ATTC20$Sol[,10])
RowanC <- (ATTC20$Sol[,1] + ATTC20$Sol[,11])
SycamoreC <- (ATTC20$Sol[,1] + ATTC20$Sol[,12])
WillowC <- (ATTC20$Sol[,1] + ATTC20$Sol[,13])

AlderB <- (ATTC20$Sol[,2] + ATTC20$Sol[,14])
AshB <- (ATTC20$Sol[,2] + ATTC20$Sol[,15])
BeechB <- (ATTC20$Sol[,2] + ATTC20$Sol[,16])
BirchB <- (ATTC20$Sol[,2] + ATTC20$Sol[,17])
ElmB <- (ATTC20$Sol[,2] + ATTC20$Sol[,18])
HazelB <- (ATTC20$Sol[,2] + ATTC20$Sol[,19])
OakB <- (ATTC20$Sol[,2] + ATTC20$Sol[,20])
RowanB <- (ATTC20$Sol[,2] + ATTC20$Sol[,21])
SycamoreB <- (ATTC20$Sol[,2] + ATTC20$Sol[,22])
WillowB <- (ATTC20$Sol[,2] + ATTC20$Sol[,23])

AlderA <- (ATTC20$Sol[,3] + ATTC20$Sol[,24])
AshA <- (ATTC20$Sol[,3] + ATTC20$Sol[,25])
BeechA <- (ATTC20$Sol[,3] + ATTC20$Sol[,26])
BirchA <- (ATTC20$Sol[,3] + ATTC20$Sol[,27])
ElmA <- (ATTC20$Sol[,3] + ATTC20$Sol[,28])
HazelA <- (ATTC20$Sol[,3] + ATTC20$Sol[,29])
OakA <- (ATTC20$Sol[,3] + ATTC20$Sol[,30])
RowanA <- (ATTC20$Sol[,3] + ATTC20$Sol[,31])
SycamoreA <- (ATTC20$Sol[,3] + ATTC20$Sol[,32])
WillowA <- (ATTC20$Sol[,3] + ATTC20$Sol[,33])
...
MeanPD <- unscale((-MeanB/(2*MeanA)), center= 146.7727, scale=14.04083)$var1
AlderPD <-  unscale((-AlderB/(2*AlderA)), center= 146.7727, scale=14.04083)$var1
AshPD <-  unscale((-AshB/(2*AshA)), center= 146.7727, scale=14.04083)$var1
BeechPD <-  unscale((-BeechB/(2*BeechA)), center= 146.7727, scale=14.04083)$var1
BirchPD <-  unscale((-BirchB/(2*BirchA)), center= 146.7727, scale=14.04083)$var1
ElmPD <-  unscale((-ElmB/(2*ElmA)), center= 146.7727, scale=14.04083)$var1
HazelPD <-  unscale((-HazelB/(2*HazelA)), center= 146.7727, scale=14.04083)$var1
OakPD <-  unscale((-OakB/(2*OakA)), center= 146.7727, scale=14.04083)$var1
RowanPD <-  unscale((-RowanB/(2*RowanA)), center= 146.7727, scale=14.04083)$var1
SycamorePD <-  unscale((-SycamoreB/(2*SycamoreA)), center= 146.7727, scale=14.04083)$var1
WillowPD <-  unscale((-WillowB/(2*WillowA)), center= 146.7727, scale=14.04083)$var1
...
#write.csv(AbundCurves,'~/Documents/Models/Tables/Inc2020/AbundCurvesMetrics.csv')
...
(PDdifplot <- ggplot(AbundCurves, aes(fct_rev(TT), (PDdif), colour=TT))+
    ...
    geom_errorbar(aes(ymax=(PDdifUCI), ymin=(PDdifLCI), width=0.2), size=1)+
    theme_bw()+
    xlab("")+
    ylab("Peak Timing (days)")+
    coord_flip()+
    ...
    theme(text = element_text(size=15), axis.text.x = element_text(angle = 45, hjust=1))+
    guides(color = "none")+
    scale_colour_manual(values=AllTaxaCols)) #5x4BES
...
(PHpropdifplot <- ggplot(AbundCurves, aes(fct_rev(TT), PHprop, colour=TT))+
    geom_point(size=3, alpha=0.5)+
    geom_errorbar(aes(ymax=PHpropUCI, ymin=PHpropLCI, width=0.2),size=1)+
    theme_bw()+
    xlab("")+
    ylab("Height (proportional)")+
    geom_hline(yintercept=1, linetype="dashed", color = "black")+
...
(PW.5difplot <- ggplot(AbundCurves, aes(fct_rev(TT), PW.5dif, colour=TT))+
  ...
  geom_errorbar(aes(ymax=PW.5difUCI, ymin=PW.5difLCI, width=0.2))+
  theme_bw()+
  xlab("")+
  ylab("Relative Width (days)")+
  coord_flip()+
...
AbundODcurves <- data.frame(TT=c("Alder", "Ash", "Beech", "Birch", "Elm", "Hazel", "Rowan", "Sycamore", "Willow"), 
                            ...
                            PHpropLCI=c(exp(HPDinterval(mcmc(log(AlderPHODprop))))[1], exp(HPDinterval(mcmc(log(AshPHODprop))))[1], exp(HPDinterval(mcmc(log(BeechPHODprop))))[1], exp(HPDinterval(mcmc(log(BirchPHODprop))))[1], exp(HPDinterval(mcmc(log(ElmPHODprop))))[1], exp(HPDinterval(mcmc(log(HazelPHODprop))))[1], exp(HPDinterval(mcmc(log(RowanPHODprop))))[1], exp(HPDinterval(mcmc(log(SycamorePHODprop))))[1], exp(HPDinterval(mcmc(log(WillowPHODprop))))[1]),
                            PHpropUCI=c(exp(HPDinterval(mcmc(log(AlderPHODprop))))[2], exp(HPDinterval(mcmc(log(AshPHODprop))))[2], exp(HPDinterval(mcmc(log(BeechPHODprop))))[2], exp(HPDinterval(mcmc(log(BirchPHODprop))))[2], exp(HPDinterval(mcmc(log(ElmPHODprop))))[2], exp(HPDinterval(mcmc(log(HazelPHODprop))))[2], exp(HPDinterval(mcmc(log(RowanPHODprop))))[2], exp(HPDinterval(mcmc(log(SycamorePHODprop))))[2], exp(HPDinterval(mcmc(log(WillowPHODprop))))[2]),
...
#write.csv(AbundODcurves,'~/Documents/Models/Tables/Inc2020/AbundCurvesODMetrics.csv')
...
#write.csv(AbundCurvesSampleSize,'~/Documents/Models/Tables/Inc2020/AbundCurvesSampleSize.csv')
...
#write.csv(AbundODCurvesSampleSize,'~/Documents/Models/Tables/Inc2020/AbundODCurvesSampleSize.csv')

meanmetrics <- data.frame(metric=c("PD", "PH", "PW.5", "PW.01"), 
                          mean=c(mean(MeanPD), exp(mean(log(MeanPH))),mean(MeanPW.5,na.rm=TRUE), mean(MeanPW,na.rm=TRUE)), 
...
#write.csv(meanmetrics,'~/Documents/Models/Tables/Inc2020/AbundFixedEffMetrics.csv')
...
#### fixed effects
fixed<-rbind(
  c("Intercept",paste(round(mean(ATTC20$Sol[,1]),3)," (",
                      round(HPDinterval(ATTC20$Sol[,1])[1],3)," - ",
                      round(HPDinterval(ATTC20$Sol[,1])[2],3),")",sep=""),
    round(effectiveSize(ATTC20$Sol[,1]))),
  c("Date (scaled)",paste(round(mean(ATTC20$Sol[,2]),3)," (",
                          round(HPDinterval(ATTC20$Sol[,2])[1],3)," - ",
                          round(HPDinterval(ATTC20$Sol[,2])[2],3),")",sep=""),
    round(effectiveSize(ATTC20$Sol[,2]))),
  c("Date² (scaled)",paste(round(mean(ATTC20$Sol[,3]),3)," (",
                           round(HPDinterval(ATTC20$Sol[,3])[1],3)," - ",
                           round(HPDinterval(ATTC20$Sol[,3])[2],3),")",sep=""),
    round(effectiveSize(ATTC20$Sol[,3]))))

#### random terms
column<-1
treetaxa1<-c("TreeTaxa- Intercept var",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                             round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                             round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
             round(effectiveSize(ATTC20$VCV[, column])))

column<-2
treetaxa2<-c("TreeTaxa- Intercept:Date slope covar",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                                          round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                                          round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
             round(effectiveSize(ATTC20$VCV[, column])))

column<-3
treetaxa3<-c("TreeTaxa- Intercept:Date² slope covar",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                                           round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                                           round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
             round(effectiveSize(ATTC20$VCV[, column])))

column<-5
treetaxa5<-c("TreeTaxa- Date slope var",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                              round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                              round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
             round(effectiveSize(ATTC20$VCV[, column])))

column<-6
treetaxa6<-c("TreeTaxa- Date slope:Date² slope covar",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                                            round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                                            round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
             round(effectiveSize(ATTC20$VCV[, column])))

column<-9
treetaxa9<-c("TreeTaxa- Date² slope var",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                               round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                               round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
             round(effectiveSize(ATTC20$VCV[, column])))

column<-10
site10<-c("Site- Intercept var",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                      round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                      round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-11
site11<-c("Site- Intercept:Date slope covar",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                                   round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                                   round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-12
site12<-c("Site- Intercept:Date² slope covar",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                                    round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                                    round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-14
site14<-c("Site- Date slope var",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                       round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                       round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-15
site15<-c("Site- Date slope:Date² slope covar",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                                     round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                                     round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-18
site18<-c("Site- Date² slope var",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                        round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                        round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-19
year10<-c("Year- Intercept var",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                      round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                      round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-20
year11<-c("Year- Intercept:Date slope covar",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                                   round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                                   round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-21
year12<-c("Year- Intercept:Date² slope covar",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                                    round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                                    round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-23
year14<-c("Year- Date slope var",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                       round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                       round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-24
year15<-c("Year- Date slope:Date² slope covar",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                                     round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                                     round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-27
year18<-c("Year- Date² slope var",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                                        round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                                        round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-31
recorder<-c("Recorder",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                             round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                             round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
            round(effectiveSize(ATTC20$VCV[, column])))
...
column<-30
siteday<-c("Site-Day",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                            round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                            round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
           round(effectiveSize(ATTC20$VCV[, column])))

column<-28
siteyear<-c("Site-Year",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                              round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                              round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
            round(effectiveSize(ATTC20$VCV[, column])))

column<-29
treeID<-c("Tree ID",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                          round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                          round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
          round(effectiveSize(ATTC20$VCV[, column])))

column<-32
residual<-c("Residual",paste(round(posterior.mode(ATTC20$VCV[, column]),3)," (",
                             round(HPDinterval(ATTC20$VCV[, column])[1],3)," - ",
                             round(HPDinterval(ATTC20$VCV[, column])[2],3),")",sep=""),
            round(effectiveSize(ATTC20$VCV[, column])))
...
write.table(rbind(c("Fixed Terms","",""),fixed,c("Random Terms","",""),random),"~/Documents/Models/Tables/Inc2020/TableATTC20.txt",sep="\t",col.names=c("","Coefficient/Variance (Mean/mode and CI)","Effective sample size"),row.names=F)
